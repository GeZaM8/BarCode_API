...
-- Dumping data for table barcode_db.kelas: ~9 rows (approximately)
REPLACE INTO `kelas` (`kelas`) VALUES
	('10 1'),
	('10 2'),
	('10 3'),
	('11 1'),
...
-- Dumping data for table barcode_db.users: ~5 rows (approximately)
REPLACE INTO `users` (`id_user`, `email`, `password`, `id_role`) VALUES
	(1, 'admin@email.com', '123', 3),
	(2, 'user1@gmail.com', '123', 1),
	(3, 'guru1@gmail.com', '123', 2),
	(4, 'user2@gmail.com', '123', 1),
...
-- Dumping data for table barcode_db.u_guru: ~1 rows (approximately)
...
CREATE TABLE IF NOT EXISTS `u_siswa` (
  ...
  `kode_jurusan` varchar(3) NOT NULL,
  `no_absen` varchar(2) NOT NULL DEFAULT '',
  `nis` varchar(10) NOT NULL,
  `nisn` int(20) NOT NULL,
  PRIMARY KEY (`id_siswa`) USING BTREE,
  UNIQUE KEY `kelas_kode_jurusan_no_absen` (`kelas`,`kode_jurusan`,`no_absen`),
  UNIQUE KEY `nis` (`nis`),
  UNIQUE KEY `nisn` (`nisn`),
  UNIQUE KEY `id_user_uniq` (`id_user`),
  KEY `id_user` (`id_user`),
  KEY `kode_jurusan` (`kode_jurusan`),
  KEY `id_kelas` (`kelas`) USING BTREE,
  ...
  CONSTRAINT `FK_u_siswa_kelas` FOREIGN KEY (`kelas`) REFERENCES `kelas` (`kelas`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `u_siswa_ibfk_1` FOREIGN KEY (`id_user`) REFERENCES `users` (`id_user`)
) ENGINE=InnoDB AUTO_INCREMENT=11 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- Dumping data for table barcode_db.u_siswa: ~3 rows (approximately)
REPLACE INTO `u_siswa` (`id_siswa`, `id_user`, `nama`, `kelas`, `kode_jurusan`, `no_absen`, `nis`, `nisn`) VALUES
	(3, 2, 'Siswa', '10 1', 'RPL', '1', '1', 1),
	(6, 4, 'a', '10 2', 'RPL', '1', '2', 2),
	(7, 13, 'a', '10 1', 'RPL', '2', '3', 3),
	(10, 17, 'a', '10 1', 'RPL', '', '4', 4);
